...
it('should return children as is if no providers are set', () => {
  ...
  render(<section data-testid={testId}>{FlatProvider.getLayout(<>{expectedInnerHtml}</>)}</section>);
...
it('should render React context providers in the correct order', () => {
  ...
  const providers: FC[] = Array.from({ length: providersCount }, (_, index: number) => {
    const context: Context<number> = createContext(index);

    const InternalComponent: FC<PropsWithChildren> = ({ children }: PropsWithChildren) => {
      const contextIndex: number = useContext(context);
      return <section {...{ [indexAttributeName]: contextIndex }}>{children}</section>;
    };
  ...
  render(
    <section data-testid={testId}>
      {providers
        .reduce(
          (flatProvider: FlatProvider, currentProvider: FC) => flatProvider.append(currentProvider),
          new FlatProvider()
        )
        .getLayout()}
    </section>
  ...
  providerInternalComponents.forEach((providerInternalComponent: Element, index: number) => {
    expect(providerInternalComponent.getAttribute(indexAttributeName)).toBe(String(index));
  });
});
...
it('should correctly handle props', () => {
  ...
  const ComponentWithProps: FC<PropsWithChildren<{ payload: object }>> = ({ payload }) => {
